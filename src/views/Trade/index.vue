  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="trade in tradeList.userAddressList"
        :key="trade.id"
      >
        <span
          :class="{ username: true, selected: trade.id === selectedArressId }"
          @click="selectedArressId = trade.id"
          >{{ trade.consignee }}</span
        >
        <p>
          <span class="s1">{{ trade.userAddress }}</span>
          <span class="s2">{{ trade.phoneNum }}</span>
          <span class="s3" v-if="+trade.isDefault">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
...
<script>
import { reqOrderTrade, reqSubmitOrder } from "../../api/trade";
export default {
  name: "Trade",
  data() {
    return {
  ...
  computed: {
    selectAddress() {
      ...
      return userAddressList
        ? userAddressList.find((trade) => trade.id === selectedArressId)
        : {};
    },
  },
  methods: {
  ...
  async mounted() {
    const tradeList = await reqOrderTrade();
    this.tradeList = tradeList;
    this.selectedArressId = tradeList.userAddressList.find(
      (adress) => adress.isDefault === "1"
    ).id;
